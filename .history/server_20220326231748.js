...
const noProgress = new Progress({
    progress: ["No progress yet, add some"]
})
...
app.get("/:personName", function (req, res) {
    ...
    Member.findById(personName, (err, per) => {
        if (!err) {
            ...
            Progress.find({}, function (err, foundProg) {

                if (foundProg.length === 0) {
                    Progress.insertMany(progressArr, function (err) {
                        if (err) {
                            console.log(err);
                        } else {
                            console.log("Successfully saved progress to DB.");
                        }
                    });
                    // res.redirect("/");
                } else {
                    Progress.deleteOne({ progress: ["No progress yet, add some"] }, (err) => {
                        if (err) return handleError(err);
                    })
                    console.log(foundProg)
            res.render('account', { personInfo: per, foundProg: foundProg.progress});
                }
            });
            console.log(per)
